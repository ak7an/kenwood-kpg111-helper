# Candidate Record Probe: Program_ID_65519.dat

- Dominant payload XOR to normalize modified file: `0x62`
- Record size shown: 32 bytes

- baseline `0x00010640`
  - hex: `cb cb cb cb cb cb cb cb cb cb cb cb cb cb cb cb cb cb cb cb cb cb cb cb cb cb cb cb cb cb cb cb`
  - raw printable: `................................`
  - bytes `+1..+14` XOR `0x34`: `��������������`
  - bytes `+19..+20`: `cb cb`
  - bytes `+19..+20` XOR `0x34`: `ff ff` LE=65535 BE=65535
- normalized modified `0x00010640`
  - hex: `cb 7d 70 6b 60 51 47 40 6b 02 01 01 05 0d 34 cb cb cb cb db cb cb cb cb cb cb cb cb cb cb cb cb`
  - raw printable: `.}pk`QG@k.....4.................`
  - bytes `+1..+14` XOR `0x34`: `ID_Test_65519`
  - bytes `+19..+20`: `db cb`
  - bytes `+19..+20` XOR `0x34`: `ef ff` LE=65519 BE=61439

